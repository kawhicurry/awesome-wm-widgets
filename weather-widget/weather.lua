...
local function worker(user_args)
	...
	--- Modified by kawhicurry
	--- get coordinates dynamicly
	-- if args.geo_api_key == nil then
	-- 	show_warning(LCLE.parameter_warning .. (args.geo_api_key == nil and "<b>geo_api_key</b>" or ""))
	-- 	return
	-- end
	-- GEO_CMD =
	-- 	[[curl --silent 'https://restapi.amap.com/v3/ip?output=json&key=0fbd7fdd93b4f085ba8f73f851658bc7' | jq -r .rectangle | awk -F ',|;' '{print ($1+$3)/2 , ($2+$4)/2 }']]
	-- coordinates = {} -- = args.coordinates
	-- awful.spawn.with_shell(GEO_CMD, function(stdout, _, _, _)
	-- 	local TEST_CMD1 = [[bash -c "echo ']] .. stdout .. [[' >> /tmp/test.txt"]]
	-- 	awful.spawn.easy_async(TEST_CMD1, _)
	-- 	local split = " "
	-- 	local mid = string.find(stdout, split)
	-- 	local pre_x = string.sub(stdout, 1, mid - 1)
	-- 	local eof = string.find(stdout, split, mid + 1)
	-- 	local pre_y = string.sub(stdout, mid + 1, eof)
	-- 	coordinates[1] = string.format("'%q'", pre_x)
	-- 	coordinates[2] = string.format("'%q'", pre_y)
	-- end)
	-- if coordinates[1] == nil then
	--   coordinates = args.coordinates
	-- end
	-- local TEST_CMD = [[bash -c "echo ']] .. coordinates[2] .. coordinates[1] .. [[' >> /tmp/test.txt"]]
	-- awful.spawn.easy_async(TEST_CMD, _)
	...
	local owm_one_cal_api = (
		...
		.. "?lat='"
		.. "\\$(echo -n $str | awk '{print $2}')"
		-- .. coordinates[1]
		.. "'&lon='"
		.. "\\$(echo -n $str | awk '{print $1}')"
		-- .. coordinates[2]
		.. "'&appid="
		.. api_key
		.. "&units="
		...
		.. LANG
    .. "';"
    .. "echo \\$(echo -n $str | awk '{print $1}') >> /tmp/test.txt"
    .. "echo \\$(echo -n $str | awk '{print $2}') >> /tmp/test.txt"
	)
